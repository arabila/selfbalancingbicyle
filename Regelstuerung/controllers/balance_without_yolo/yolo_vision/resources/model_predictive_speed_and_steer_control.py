...
from resources import cubic_spline_planner
NX = 4  # x = x, y, v, yaw
NU = 2  # a = [accel, steer]
T = 2  # horizon length
...
# TODO: Play with this setting, probably reduce bc steering on bike uses Roll as well
...
def calc_nearest_index(state, cx, cy, cyaw, pind):
    """
    Calcuclates nearest point to initial state of model. This can be left out and i[0] can be used
    :param state:
    :param cx:
    :param cy:
    :param cyaw:
...
def do_simulation(cx, cy, cyaw, ck, sp, dl, initial_state):
    ...
    # we dont need this
    # TODO: Trim down function to increase speed
    goal = [cx[-1], cy[-1]]
    ...
    while MAX_TIME >= time:
        ...
        state = update_state(state, ai, di)
        # TODO: break here, return state, connect to BBB, set v, set yaw
        time = time + DT
...
def main(ax, ay, state):
    ...
    t, x, y, yaw, v, d, a = do_simulation(cx, cy, cyaw, ck, sp, dl, state)
