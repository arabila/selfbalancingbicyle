...
# Lade Konfiguration und starte Monitoring
config_loader = ConfigLoader()
monitoring_logger = MonitoringLogger()

# Lade Parameter aus Konfiguration
preview = config_loader.get_parameter('preview', 1)
Kp = config_loader.get_parameter('Kp', 0.01)
Ki = config_loader.get_parameter('Ki', 0.02)
Kd = config_loader.get_parameter('Kd', 0.0001)
maxS = config_loader.get_parameter('maxS', 5.0)
minS = config_loader.get_parameter('minS', 3.0)
hMax = config_loader.get_parameter('hMax', 0.1920)
...
def printStatus():
    ...
    if robot.getName() == 'Little Bicycle 1':
        ...
        vpos = 0.89  # Setze hier den gewünschten Wert für 'Little Bicycle V2'
    else:
        vpos = 0.85  # Optional: Setze einen Standardwert für unerwartete Namen
...
while robot.step(timestep) != -1:
    ...
    # Lade Konfiguration alle 10 Schritte neu
    config_reload_counter += 1
    if config_reload_counter >= 10:
        config_loader.reload_config()
        Kp = config_loader.get_parameter('Kp', Kp)
        Ki = config_loader.get_parameter('Ki', Ki)
        Kd = config_loader.get_parameter('Kd', Kd)
        maxS = config_loader.get_parameter('maxS', maxS)
        minS = config_loader.get_parameter('minS', minS)
        hMax = config_loader.get_parameter('hMax', hMax)
        preview = config_loader.get_parameter('preview', preview)
        config_reload_counter = 0
    ...
    # Hole Geschwindigkeit für Monitoring
    velo = robotNode.getVelocity()
    velocity = (velo[0]**2 + velo[1]**2 + velo[2]**2)**0.5
    velocity = velocity * 3.6  # km/h
