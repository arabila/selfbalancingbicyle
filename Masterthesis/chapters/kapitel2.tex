...
linearen Regelungstechnik erläutert und ihre Eignung zur Stabilisierung des unteraktuierten Balance-Problems des Fahrrads diskutiert. 
...
Stellgrößen erlaubt und somit für die Fahrtrichtungsregelung prädestiniert ist. Abschließend werden die Grundlagen der Objekt­erkennung 
...
Ansprechverhalten und Stabilitätsreserve:  Eine Erhöhung des proportionalen Anteils verringert die Regelabweichung, kann aber zu
 Instabilitäten führen;  ein zu hoher Integrationsanteil kann Integrator‑Windup verursachen;  die derivativen Anteile sind besonders rauschanfällig.
...
Ein Rad, dessen Schwerpunkt über der Aufstandsfläche liegt, befindet sich in einem instabilen Gleichgewicht;  kleinste Störungen 
führen zum Kippen.  Um das System aufrecht zu halten, muss der Regler in Echtzeit Stellmomente applizieren, die den kippspezifischen 
...
einfachen Implementierung auf Mikrocontrollern und der guten Robustheit gegenüber Modellunsicherheiten geeignet.  Die Nichtlinearität 
...
frühzeitig gegensteuern und gleichzeitig harte Grenzen, etwa Stellweg‑ oder Geschwindigkeitsbeschränkungen, einhalten \cite{do_mpc_basics}.
...
Stellgrößen \(\delta\in[-\delta_{\max},\delta_{\max}]\) und \(a\) berechnet; andernfalls fällt der Controller auf eine einfache P‑Regelung zurück.  
Vor der Weitergabe an den schnellen Balance‑Controller werden die resultierenden 


Lenkwinkel und Beschleunigungen normiert, geglättet und auf Änderungsraten begrenzt, um ruckfreie Übergänge zu gewährleisten \cite{vision_mpc_readme}.
...
Das Fahrradsystem ist nichtlinear und unteraktuiert; um einer gekrümmten Spur zu folgen, muss der Regler zukünftige Kurvenverläufe 
...
dabei die mechanischen Limits einhielt.  Somit stellt MPC einen geeigneten Ansatz dar, um das autonome Fahrrad stabil und vorausschauend \cite{wikipedia_mpc}
...
Bounding Boxes für alle Objekte in einem Bild zu bestimmen \cite{yolo_wikipedia}.  Neuere Versionen wie YOLOv8 erweitern diese Architektur und bieten 
neben der Objekterkennung auch Bildklassifikation und Instanzsegmentierung; sie verwenden eine moderne Backbone‑Struktur, einen ankerfreien Detektionskopf 
und eine verbesserte Verlustfunktion und sind auf verschiedenen Hardwareplattformen effizient einsetzbar \cite{openmmlab_yolov8}.
...
YOLOv8 integriert eine Instanzsegmentierungskomponente, deren Kopf auf dem YOLACT‑Konzept 
basiert und die zu den Detektionsausgängen zusätzliche Masken pro 
Objekt berechnet \cite{openmmlab_yolov8}.  Durch diese Erweiterung kann ein Modell in einem einzigen Forward‑Pass sowohl Objekte lokalisieren als auch 
...
(„meadow“), Hindernis („obstacle“), Hauptfahrbahn („street\_main“) und Seitenfahrbahn („street\_side“) \cite{yolo_vision_regelung} – und liefert 
