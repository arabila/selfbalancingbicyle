...
aber langsame Dynamik verursachen \citep{apmonitor_pid}.  Die Derivative wirkt der Änderung des Fehlers entgegen und dämpft dadurch Überschwinger. Sie 
...
Neigungswinkel mittels IMU messen und über den Lenkmotor oder den Antrieb korrigieren \citep{concurrent_rt_inverted_pendulum}.
...
Abbildung \ref{fig:mpc_scheme_basic} verdeutlicht das fundamentale Prinzip der modellprädiktiven Regelung. Das Schema zeigt die charakteristische 
Zeitaufteilung zwischen Vergangenheit (PAST) und Zukunft (FUTURE), wobei der aktuelle Zeitpunkt \(k\) die Grenze bildet. Die rote 
Referenztrajektorie stellt den gewünschten Sollverlauf dar, während die gelbe Linie den vorhergesagten Systemausgang (Predicted Output) 
über den Prädiktionshorizont \(p\) zeigt. Besonders deutlich wird die Diskrepanz zwischen der orangenen Linie (Measured Output), die den 
tatsächlich gemessenen Systemausgang repräsentiert, und der Vorhersage. 

Die türkise Linie zeigt den vorhergesagten Stellgrößenverlauf (Predicted Control Input), während die dunkelblaue Linie den bisherigen 
Stellgrößenverlauf (Past Control Input) darstellt. Ein zentrales Merkmal von MPC wird durch den rechteckigen Verlauf der Stellgrößen 
deutlich: Die Optimierung erfolgt in diskreten Zeitschritten, und die berechneten Stellgrößen werden über die jeweiligen Abtastintervalle 
konstant gehalten. Der Prädiktionshorizont erstreckt sich vom aktuellen Zeitpunkt \(k\) bis zum Zeitpunkt \(k+p\), wobei das System sein 
zukünftiges Verhalten basierend auf dem internen Modell vorhersagt und die Stellgrößensequenz entsprechend optimiert.
...
begrenzt, um ruckfreie Übergänge zu gewährleisten \citep{vision_mpc_readme}. Eine spezifische erklärung der implementierten Regelung findet sich in Kapitel 5.2.
...
2016 von Redmon et al. vorgestellt und erfordert im Gegensatz zu region‑proposal‑basierten Ansätzen nur einen Vorwärtsdurchlauf durch das Netz, um Klassen und 
Bounding Boxes für alle Objekte in einem Bild zu bestimmen \citep{redmon2016you}.  Neuere Modelle wie YOLOv8 erweitern die ursprüngliche YOLO-Architektur. 
...
Verlustfunktionen erhöhen Effizienz und Genauigkeit. Dadurch laufen die Modelle auf vielen Hardwareplattformen zuverlässig und schnell \citep{openmmlab_yolov8}.
...
einzelnen Instanzen zu unterscheiden, während Instance Segmentation jedes Pixel einer individuellen Objektinstanz zuordnet.  
Panoptic Segmentation kombiniert beide Ansätze, indem sie sowohl die Klasse als auch die Instanz für jedes Pixel identifiziert. 
 ...
 basierender Kopf für jedes erkannte Objekt eine zusätzliche Maske berechnet \citep{openmmlab_yolov8}.  
...
um den Kompromiss zwischen Geschwindigkeit und Genauigkeit zu steuern \citep{openmmlab_yolov8}.
...
(„meadow“), Hindernis („obstacle“), Hauptfahrbahn („street\_main“) und Seitenfahrbahn („street\_side“), wie in Abbildung \ref{fig:yolo} zu sehen ist \citep{vision_mpc_readme}. Dabei liefert es
